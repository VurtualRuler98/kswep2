...
util.AddNetworkString("kswep_reloadmessage")
util.AddNetworkString("kswep_stabsound")
util.AddNetworkString("kswepfirebulletclient")
util.AddNetworkString("kswep_medical")
util.AddNetworkString("kswep_gunshot")
...
net.Receive("kswep_2dscopesetup",function(len,pl)
	local wep=net.ReadEntity()
	local scope=net.ReadString()
	if (wep.Owner~=pl) then return end --TODO limit this function's power
	wep:SetOptic2D(scope)
end)
